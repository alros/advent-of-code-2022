...
public class Folder {
	...
	public int getTotalSize() {
		return folders.stream().map(f -> f.getTotalSize()).reduce((a, b) -> a + b).orElseGet(() -> 0) + this.size;
	}
	...
	public int findFolderBetween(int min, int max) {
		...
		if (folders.size() == 0) {
			return totalSizeCurrentBest > min && totalSizeCurrentBest < max ? totalSizeCurrentBest : MAX_VALUE;
