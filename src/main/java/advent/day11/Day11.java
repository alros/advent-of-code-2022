...
public class Day11 {
  ...
  public long solve(BufferedReader br, int loops, int factor) throws IOException {
  ...
  private Monkey parseMonkey(BufferedReader br) throws IOException {
    return Monkey.builder()
        .worryLevel(
            new ArrayList<>(
                Arrays.stream(gerFirstLine(br).split(":")[1].split(","))
                    .map(n -> Long.parseLong(n.trim()))
                    .toList()))
  ...
  private void play(Monkey monkey, List<Monkey> monkeys, int factor, int lcm) {
    while (!monkey.getWorryLevel().isEmpty()) {
      long newWorryLevel =
          calculate(
              monkey.getOperation().replaceAll("old", monkey.getWorryLevel().remove(0).toString()),
              factor);
      monkeys
          .get(
              newWorryLevel % monkey.getDivisible() == 0
                  ? monkey.getMonkeyTrue()
                  : monkey.getMonkeyFalse())
          .getWorryLevel()
          .add(newWorryLevel % lcm);
      monkey.incInspected();
    }
  }
