...
public class Day11 {
	...
	public int solve(BufferedReader br, int loops, int divideWorry) throws IOException, ScriptException {
		...
		for (int round = 0; round < loops; round++) {
			...
			for (int i = 0; i < monkeyId; i++) {
				...
				monkeys.values().stream().sorted((a, b) -> a.getId() - b.getId()).forEach(System.out::println);
				System.out.println("====");
			}
		}

		List<Monkey> mostActive = monkeys.values().stream().sorted((a, b) -> b.getInspected() - a.getInspected())
				.limit(2).toList();
	...
	private Monkey parseMonkey(int id, BufferedReader br) throws IOException {
		...
		List<Integer> worryLevel = Arrays.stream(line.split(":")[1].split(",")).map(n -> Integer.parseInt(n.trim()))
				.toList();
	...
	private void play(int monkeyId, Map<Integer, Monkey> monkeys, int divideWorry) throws ScriptException {
		...
		for (; !monkey.getWorryLevel().isEmpty();) {
			...
			Integer newWorryLevel = ((Integer) scriptEngine
					.eval(monkey.getOperation().replaceAll("old", worryLevel.toString()))) / divideWorry;
