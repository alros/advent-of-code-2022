...
public class Day11 {
	...
	public long solve(BufferedReader br, int loops, int divideWorry) throws IOException, ScriptException {
	...
	private Monkey parseMonkey(int id, BufferedReader br) throws IOException {
		...
		List<Long> worryLevel = Arrays.stream(line.split(":")[1].split(",")).map(n -> Long.parseLong(n.trim()))
				.toList();
	...
	private void play(int monkeyId, Map<Integer, Monkey> monkeys, int divideWorry, int lcm) throws ScriptException {
		...
		for (; !monkey.getWorryLevel().isEmpty();) {
			...
			Long newWorryLevel = calc(monkey.getOperation().replaceAll("old", worryLevel.toString()), divideWorry);
