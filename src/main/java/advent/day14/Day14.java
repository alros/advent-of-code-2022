...
public class Day14 {
	...
	private int[][] parse(BufferedReader br, boolean endless) throws IOException {
		...
		for (String line = br.readLine(); line != null; line = br.readLine()) {
			lines.add(Arrays.stream(line.split(" -> ")).map(s -> {
				String[] sp = s.split(",");
				return new Point(parseInt(sp[0]), parseInt(sp[1]));
			}).toList());
		}
		int maxX = lines.stream().flatMap(l -> l.stream()).sorted((a, b) -> b.x() - a.x()).findFirst().get().x() + 1;
		int maxY = lines.stream().flatMap(l -> l.stream()).sorted((a, b) -> b.y() - a.y()).findFirst().get().y() + 1;
		...
		lines.forEach(l -> {
			...
			for (int i = 1; i < l.size(); i++) {
				...
				for (int x = (start.x() < next.x() ? start : next).x(); x < (start.x() < next.x() ? next : start)
						.x(); x++) {
					map[start.y()][x] = ROCK;
				}
				for (int y = (start.y() < next.y() ? start : next).y(); y < (start.y() < next.y() ? next : start)
						.y(); y++) {
					map[y][start.x()] = ROCK;
				}
				start = next;
