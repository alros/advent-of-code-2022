...
public class Day12 {
	...
	public int solveStep2(BufferedReader br) throws IOException {
		List<String> map = parseMap(br);
		return locatePosition(map, "a").stream().map(start -> explore(map, start)).sorted().findFirst().get();
	}
	...
	public List<int[]> locatePosition(List<String> map, String mark) {
		return Streams.mapWithIndex(map.stream(), (string, y) -> new int[] { (int) y, string.indexOf(mark) })
				.filter(pair -> pair[1] >= 0).toList();
	}

	public boolean canMove(List<String> map, Point start, Point next) {
		if (next.x() < 0 || next.y() < 0 || map.size() <= next.y() || map.get(next.y()).length() <= next.x()) {
			return false;
		}
